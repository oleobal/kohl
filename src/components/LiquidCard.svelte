<script lang="ts">
  ...
  import type { Liquid, NormalizedLiquid, ErrorLiquid, LiquidMakeup, NormalizedLiquidMakeup } from "../lib/liquid";
...
<div class="container">
  <div class="top-bar">
  <input class="value" id="in-temp-{id}" type=number bind:value={liquid.temp} placeholder={valueOrError("temp")}/>
  <button id="btn-close-{id}">X</button>
  </div>
  <div class={{
    "inside": true,
