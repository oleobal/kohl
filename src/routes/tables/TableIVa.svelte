...
<tbody>
{#each {length: 101}, abv}
  <tr class={{ "even-row": (abv%10<5)}} style="text-align: right; font-variant-numeric: lining-nums;">
  <td>
    <strong>{abv}</strong>
  </td>
  {#each decimals as decimal}
      ...
      <td
        ...
        title={abv+decimal/10+ " %vol → " + calculatedTable.getDensityFromABV(abv+decimal/10, 20)+" g/L"}
      >
        {calculatedTable.getDensityFromABV(abv+decimal/10, 20).toFixed(2)}
      </td>
      {/if}
